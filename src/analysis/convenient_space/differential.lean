...
namespace convenient
  ...
  section differential
    ...
    namespace detail
      ...
      def diff : (((E⟿F)×E)×E)⟿F := ⟨diff.to_fun, diff.is_smooth⟩
    ...
    def diff : (E⟿F)⟿(E⟿(E⟿F)) := (detail.diff).curry.curry

    def Diff : (E⟿F)⊸(E⟿(E⊸F)) := sorry

    notation δ f := diff f

    lemma diff.arg3.is_linear {f : E⟿F} {x : E} : is_linear_map ℝ (δ f x) := sorry
    @[simp] lemma Diff_diff {f : E⟿F} {x dx : E} : Diff f x dx = δ f x dx := sorry
